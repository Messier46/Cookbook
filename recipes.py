    ...
    def add(self):
        ...
        while self.rerun:
            ...
            for x in range(self.amount):
                singleIng = ''
                singleIng = input(
                    "What is the ingredient and it's amount? (Press enter after each ingredient and qty added)\n")
                while singleIng == '':
                    singleIng = input(
                        "Blank input. What is the ingredient and it's amount?"
                        " (Press enter after each ingredient and qty added)\n")
            ...
            print(
                '\n\nName %s \nType %s \nIngredients %s \nBake Time %s \nBake Temp %i \n\nDirections %s \nFavorite %s' % (
                self.name, self.recipeType, self.ingString, self.bakeTime, self.bakeTemp, self.directions,
                self.favPick,))

            clearUp = input(
                'Does this look correct? If yes type in yes to continue, else type no to retype the recipe\n')
            while clearUp != 'no' and clearUp != 'yes':
                clearUp = input(
                    'Incorrect input. Does this look correct? If yes type in yes to continue,'
                    ' else type no to retype the recipe\n')
